...
/// parsed results and only re-parse files that actually changed. Persisted to
/// disk so the index survives a daemon/agent restart (warm start).
...
fn mtime_to_millis(mtime: Option<SystemTime>) -> Option<u64> {
    mtime
...
fn outline_cache_ensure_loaded(project_root: &Path) {
...
/// still match.
fn outline_cache_get(path: &Path, mtime_ms: Option<u64>, size: u64) -> Option<Vec<SymbolEntry>> {
...
fn outline_cache_put(path: &Path, mtime_ms: Option<u64>, size: u64, symbols: Vec<SymbolEntry>) {
    let Ok(mut state) = OUTLINE_CACHE.lock() else {
        return;
    };
...
fn outline_file(project_root: &Path, path: &Path, limit: usize) -> PeriResult<Vec<SymbolEntry>> {
    ...
    outline_cache_ensure_loaded(project_root);
    ...
    let full = match outline_cache_get(path, mtime_ms, size) {
        Some(cached) => cached,
        None => {
            let built = build_outline_full(project_root, path)?;
            outline_cache_put(path, mtime_ms, size, built.clone());
            built
        }
    };
