...
use commands::{
    ...
    read_stored_api_key, read_stored_openai_oauth_credentials, run_agents_command,
...
fn apply_session_command(
    ...
    match command {
        ...
        SessionCommandEvent::BranchSave(name) => {
            tui_branch::handle_branch_save(state, project_template, &name);
        }
        SessionCommandEvent::BranchRestore(name) => {
            tui_branch::handle_branch_restore(state, project_template, &name);
        }
        SessionCommandEvent::BranchList => {
            tui_branch::handle_branch_list(state, project_template);
        }
        SessionCommandEvent::BranchTurn(turn_id) => {
            tui_branch::handle_branch_turn(state, project_template, turn_id);
        }
        SessionCommandEvent::BranchTree => {
            tui_branch::handle_branch_tree(state, project_template);
        }
        SessionCommandEvent::BranchSwitch(index) => {
            tui_branch::handle_branch_switch(state, project_template, index);
        }
        ...
        SessionCommandEvent::Skill { name, args } => {
            tui_skill::handle_skill_load(state, project_template, &name, &args);
        }
        ...
        SessionCommandEvent::SkillList => {
            tui_skill::handle_skill_list(state, project_template);
        }
        SessionCommandEvent::SkillShow(name) => {
            tui_skill::handle_skill_show(state, project_template, &name);
        }
        SessionCommandEvent::SkillSearch(query) => {
            tui_skill::handle_skill_search(state, project_template, &query);
        }
        SessionCommandEvent::SkillArchived(query) => {
            tui_skill::handle_skill_archived(state, project_template, &query);
        }
        SessionCommandEvent::SkillPin(name) => {
            tui_skill::handle_skill_pin(state, project_template, &name, true);
        }
        SessionCommandEvent::SkillUnpin(name) => {
            tui_skill::handle_skill_pin(state, project_template, &name, false);
        }
        SessionCommandEvent::SkillArchive(name) => {
            tui_skill::handle_skill_archive(state, project_template, &name);
        }
        SessionCommandEvent::SkillRestore(name) => {
            tui_skill::handle_skill_restore(state, project_template, &name);
        }
...
pub(crate) fn context_snapshot_path(project_root: &Path, session_id: &str) -> PathBuf {
    project_root
        .join(".peridot/sessions")
        .join(session_id)
        .join("context.bin")
...
pub(crate) fn branch_journal_path(project_root: &Path, session_id: &str) -> PathBuf {
    project_root
        .join(".peridot/sessions")
        .join(session_id)
        .join("branches.json")
...
pub(crate) fn append_plan_reminder_to_context(
    project_root: &Path,
    session_id: &str,
    content: String,
...
pub(crate) fn load_auto_skill_suggestions(project_root: &Path) -> Vec<SkillSlashSuggestion> {
    ...
    records
        ...
        .map(|skill| SkillSlashSuggestion {
            ...
            description: tui_skill::skill_description(&skill),
            name: skill.name,
        })
        .collect()
}
...
/// Loads the shared code-map TODO index and prints `path:line:
/// trimmed-text` for each hit. The index refreshes automatically when
/// source files change or when an older cache was built with a lower
/// TODO cap.
fn handle_scan_todos(state: &mut TuiState, project_root: &Path) {
    let load = commands::load_or_refresh_code_map_index_with_status(
        project_root,
        commands::DEFAULT_MAX_SYMBOLS,
        commands::TODO_SCAN_MAX_TODOS,
    );
