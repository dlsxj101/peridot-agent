...
//!
//! This crate exists so that both `peridot-verify` (CLI verify pipeline)
//! and `peridot-core` (agent harness `auto_grade_on_done`) can call the
//! same grader without creating a dependency cycle — verify was previously
//! LLM-free, and core owned both the harness and the LLM provider trait.
//! `peridot-core` re-exports the public API from this crate for
//! backward-compatibility with callers that imported `peridot_core::grade_work`.
