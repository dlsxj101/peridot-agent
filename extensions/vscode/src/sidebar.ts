...
export interface SidebarHandlers {
  ...
  newPersistedSession: () => Promise<void>;
  switchPersistedSession: () => Promise<void>;
  closePersistedSession: () => Promise<void>;
  ...
  renamePersistedSession: () => Promise<void>;
...
export class PeridotSidebarProvider implements vscode.WebviewViewProvider {
  ...
  public appendCommandResult(result: CommandResultView): void {
    ...
    const codeMap = codeMapFromCommandResult(result, this.state.context.codeMap);
    if (codeMap) {
      this.state.context = {
        ...this.state.context,
        codeMap,
  ...
  private async receive(message: OutboundMessage): Promise<void> {
    switch (message.type) {
      ...
      case 'showTodos':
        ...
        return;
      case 'showContextTop':
        await this.handlers.showContextTop();
        return;
      case 'showWorkingTreeDiff':
        await this.handlers.showWorkingTreeDiff();
        return;
      case 'showMcpServers':
      ...
      case 'addSessionNote':
        ...
        return;
      case 'showSessionNotes':
        await this.handlers.showSessionNotes();
        return;
      case 'clearSessionNotes':
        await this.handlers.clearSessionNotes();
        return;
      case 'newPersistedSession':
        await this.handlers.newPersistedSession();
        return;
      case 'switchPersistedSession':
        await this.handlers.switchPersistedSession();
        return;
      case 'closePersistedSession':
        await this.handlers.closePersistedSession();
        return;
      case 'showSessionCount':
        await this.handlers.showSessionCount();
        return;
      case 'showPersistedSessionDetails':
        await this.handlers.showPersistedSessionDetails();
        return;
      ...
      case 'resumePersistedSession':
        ...
        return;
      case 'renamePersistedSession':
        await this.handlers.renamePersistedSession();
        return;
      case 'deletePersistedSession':
        await this.handlers.deletePersistedSession();
        return;
      case 'showSessions':
