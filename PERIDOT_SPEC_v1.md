...
- 🚧 Multimodal vision (F2 첫 증분) — `peridot-llm`의 `LlmMessage`가 추가형 `images` 필드(+`user_with_images`)를 갖고, Anthropic·OpenAI Chat·OpenAI Codex 어댑터가 이미지를 각 네이티브 블록으로 직렬화하며, `model_supports_vision` capability 게이트 추가(텍스트 경로 불변). 남은 첨부→이미지 resolver/라우팅/OCR 폴백은 [F2 설계 문서](docs/plans/f2-multimodal-vision.md) milestone 4~6 참조
