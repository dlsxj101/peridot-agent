...
- ✅ **Auto-fix loop (verify pass → fix → re-verify)** — `VerifyFailureState` 시그니처 추적 + 사이클 카운팅
  ...
  단계에서 진짜 루프로 승격됨.
- 🚧 LSP / 심볼 인덱스 (F1 첫 증분) — `peridot-symbols` 크레이트가 `tree-sitter-rust`로 Rust 심볼(kind/이름/라인범위/impl·trait container)을 구조적으로 추출하고, `file_outline` / `workspace_symbols` / `symbol_search` 도구가 `.rs` 파일에 이를 사용. 향후 `symbol_definition`/`symbol_references` 전용 도구, 추가 언어 grammar, 증분 갱신은 [docs/plans/roadmap-v1.0.md](docs/plans/roadmap-v1.0.md) F1 참조
