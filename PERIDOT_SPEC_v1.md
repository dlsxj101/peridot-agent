...
- ✅ `@file` auto-mention — `peridot-tui::at_picker` 모듈 + slash-picker 패턴, VS Code composer parity, long-lived session file-index refresh, `/attach <path>`와 `/codemap outline <path>` slash argument path autocomplete, attached-path 기반 `/detach <path>` autocomplete
...
- ✅ TODO/FIXME 인덱스 조회 — `/todos` 슬래시가 `.peridot/codemap.json`의 TODO marker 인덱스를 재사용하고, stale 또는 낮은-cap 인덱스는 자동 refresh
...
- ✅ **Auto-fix loop (verify pass → fix → re-verify)** — `VerifyFailureState` 시그니처 추적 + 사이클 카운팅
  ...
  단계에서 진짜 루프로 승격됨.
- 🚧 LSP / 심볼 인덱스 (F1) — `peridot-symbols` 크레이트가 tree-sitter로 **Rust·TypeScript/JavaScript/JSX·Python·Go·Java·Ruby·C·C++·C#·PHP·Bash·Scala·Lua** 심볼(kind/이름/라인범위/container)과 식별자 references를 구조적으로 추출(확장자 디스패처). `file_outline` / `workspace_symbols` / `symbol_search`가 지원 확장자에 이를 사용하고, 전용 `symbol_definition`(정확한 이름의 정의)·`symbol_references`(지원 언어는 AST 기반으로 주석·문자열 제외; 그 외는 단어경계 텍스트 폴백) 도구를 등록. 추가 언어(Go/Java/…) grammar, scope-aware references, 증분 갱신은 [docs/plans/roadmap-v1.0.md](docs/plans/roadmap-v1.0.md) F1 참조
- 🚧 Multimodal vision (F2) — `peridot-llm`이 이미지를 운반(추가형 `images` 필드 + `user_with_images`)하고 Anthropic·OpenAI Chat·OpenAI Codex 어댑터가 네이티브 블록으로 직렬화. **End-to-end**: `/attach`가 이미지를 base64 인코딩(≤5MB)해 `ContextEntry.images`에 싣고, `to_messages`가 `user_with_images`를 방출, core 루프가 `model_supports_vision` 게이트로 텍스트 전용 모델에는 이미지를 strip. 남은 다운스케일/OCR 폴백/설정 노브는 [F2 설계 문서](docs/plans/f2-multimodal-vision.md) milestone 5~6 참조
- ⏸️ 음성 / Web UI — 별 프로젝트 (v2). 설계 문서: [F3](docs/plans/f3-voice-input.md) / [F4](docs/plans/f4-web-ui.md)
...
- [x] 문서: 사용자 가이드 ([docs/user-guide.md](docs/user-guide.md)), 기여 가이드 ([CONTRIBUTING.md](CONTRIBUTING.md))
- [x] 라이선스: MIT (루트 [LICENSE](LICENSE), `Cargo.toml` `license = "MIT"`, 확장 [extensions/vscode/LICENSE](extensions/vscode/LICENSE)와 일치)
