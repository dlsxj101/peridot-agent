...
- ✅ **Auto-fix loop (verify pass → fix → re-verify)** — `VerifyFailureState` 시그니처 추적 + 사이클 카운팅
  ...
  단계에서 진짜 루프로 승격됨.
- 🚧 LSP / 심볼 인덱스 (F1) — `peridot-symbols` 크레이트가 tree-sitter로 **Rust·TypeScript/JavaScript/JSX·Python·Go·Java·Ruby·C·C++** 심볼(kind/이름/라인범위/container)과 식별자 references를 구조적으로 추출(확장자 디스패처). `file_outline` / `workspace_symbols` / `symbol_search`가 지원 확장자에 이를 사용하고, 전용 `symbol_definition`(정확한 이름의 정의)·`symbol_references`(지원 언어는 AST 기반으로 주석·문자열 제외; 그 외는 단어경계 텍스트 폴백) 도구를 등록. 추가 언어(Go/Java/…) grammar, scope-aware references, 증분 갱신은 [docs/plans/roadmap-v1.0.md](docs/plans/roadmap-v1.0.md) F1 참조
