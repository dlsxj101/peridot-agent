# Peridot Agent — Architecture Specification v1.6
...
구현:
  ...
  peridot-llm          멀티 모델 라우팅 (main, goal_checker, compaction)
...
완료 기준:
  ...
  git tag v0.1.0 → CI가 바이너리 빌드 + Release 생성
```
...
### 21.5.9 v1 작업 중 구현 완료한 Beyond-v1 항목 (2026-05-17)

- ✅ `@file` auto-mention — `peridot-tui::at_picker` 모듈 + slash-picker 패턴
- ✅ GitHub PR workflow 기초 — `gh_pr_create` / `gh_pr_status` / `gh_pr_merge` 도구 (peridot-tools/src/tools/git.rs)
- ✅ Conversation branching (파일 기반) — `/branch save|restore|list` 슬래시. 진정한 turn-level fork는 별도 작업
...
#### 진정한 turn-level conversation branching
- Per-turn context boundary tracking in `ContextManager`
- DAG-shaped session journal (현재는 linear append-only)
- TUI에서 분기 visualization (트리뷰)
- 예상 작업량: 1-2주

#### Diff staging / 부분 수락
- Approval Panel 재설계: file_patch diff를 hunk 단위로 분할
- 각 hunk 별 accept/reject UI
- 선택된 hunk만 적용하는 patch 합성 로직
- 예상 작업량: 1주

#### Auto-fix loop (verify pass → fix → re-verify)
- 현재는 recovery directive로 prompting만 함. 진정한 loop는 verify_test/verify_lint이 fail하면 자동으로 다음 턴 진입 + 명시적 "fix" 지시
- StuckDetector와 상호작용 신중히 설계 필요 (무한 루프 방지)
- 예상 작업량: 3-5일
