...
- Semantic symbol extraction (`peridot-symbols`, tree-sitter) for Rust, TypeScript/JavaScript/JSX, Python, Go, Java, Ruby, C, C++, C#, PHP, and Bash, backing `file_outline` / `workspace_symbols` / `symbol_search` / `symbol_definition` / `symbol_references`; other languages use the line-based heuristic.
