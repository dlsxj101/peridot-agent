...
- **`peridot-context/src/lib.rs` carved down** (~2,618 → ~2,219 lines): the
  ...
  unchanged.
- **`peridot-core/src/agent.rs` carved down** (~3,001 → ~2,581 lines): the
  ...
  policy imports the moved items from `crate::verify_failure`), the tool-call
...
- **`peridot-tui/src/state.rs` carved down** (~3,176 → ~2,903 lines): the
...
- **`peridot-cli/src/main.rs` carved down** (~4,774 → ~4,097 lines): the
  interactive `/codemap` slash-command handlers and report renderers moved into
  a new `tui_codemap.rs` module, the `/branch` slash-command handlers into a new
  `tui_branch.rs` module, and the `/session export` handlers + import-result
  renderer into a new `tui_session_export.rs` module. No behavior change.
...
- **VS Code session resume GUI** now exposes `/session resume <id>` from the
...
- **VS Code MCP server list GUI** now exposes `/mcp list` from the command
  palette and sidebar title bar, rendering configured server rows through the
  existing structured command-result renderer.
- **VS Code MCP server add GUI** now exposes `/mcp add <name> <transport>
...
- **VS Code runtime control GUI** now exposes execution mode, permission
...
- **TUI session export rendering** now lists full-copy session file names in
  the `/export` transcript, matching the CLI and VS Code export details.
- **Session export command-result rows** now include full-copy session files in
...
- **VS Code session menu context subtitles** now include persisted Notes and
  Attachments summaries beside status and usage totals in the sidebar session
  switcher.
